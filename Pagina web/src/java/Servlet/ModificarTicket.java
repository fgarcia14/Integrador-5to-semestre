...
public class ModificarTicket extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Ticket t = new Ticket(noTicket, 0, "", departamento, asunto, mensaje, "", "");
        ControladorTicket ct = new ControladorTicket();
        
        if(ct.updateTicket(t)){
            response.getWriter().println("Ticket modificado");
        }else{
            response.getWriter().println("ERROR al modificar ticket");
        }
    }
